...
`master` pushes trigger `.github/workflows/master_fishingmap.yml` (re-enabled and fixed July 2026 after years disabled), which runs install, lint, build, and test as separate steps on Windows (separate because multi-line pwsh `run:` blocks only propagate the last command's exit code), then deploys **only the production build** to Azure Web App `fishingmap`. The artifact holds the `build/` folder's **contents** and the deploy job downloads them to the package root — the Azure app serves wwwroot directly, so `index.html` and the SPA rewrite `web.config` (shipped via `public/`) must land at the top level. **Do not re-nest the artifact under `build/`**: that was tried and served a 403 at the site root (fixed in `d25f746`). Sources and `node_modules` are never deployed. The build uses Node 20; `.env.production` is consumed at build time only (Vite bakes the values into the bundle). Manual fallback deploy: `npm run build`, then deploy the `build/` folder with the VS Code Azure extension — same result.
